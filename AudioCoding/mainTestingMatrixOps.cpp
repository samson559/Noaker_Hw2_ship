...
void DCTvector(int N, int k, float* q)
{
	...
	for (int i = 0; i < N; i++)
	{
		q[(i*N+k)] = sk*cos(pifrac*k*((2 * ind) - 1));
		ind++;
	}
}
...
/*
I wrote some helpers for testing
*/
void OutputMatrix(const float * m, const int sz)
{
	for (int i = 0; i < sz; i++)
	{
		printf("\n");
...
void transposeMatrix(float* mt, float * m, const int sz)
{
	for (int i = 0; i < sz; i++)
	{
		for (int j = 0; j < sz; j++)
...
int main()
{
	...
	float q[] = { -0.3925, 0.8478, 0.3565,
	-0.7150, -0.5251, 0.4616,
	-0.5786, 0.0737, -0.8123,
	};
	
	float * qt = new float[diag * diag];
	/*
	for (int i = 0; i < diag;i++)
		DCTvector(diag, i, q+(i*diag));
	*/
	...
	float*res;
	res = mult_matrix(q, qt, diag);
		printf("qqt=\n");
	OutputMatrix(res, diag);
